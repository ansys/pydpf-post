...
# -----------------------------------------------------------------------------------------
# Extract results
# print(static_simulation._model.metadata.time_freq_support)

# Extract displacements along X for nodes 1, 2 and 3 at set 1
displacement_X = static_simulation.displacement(
    components=["X"], nodes=[1, 2, 3], set_ids=[1]
)
print(displacement_X._fc)
...
# Should an "elements" input be considered and a field on equivalent nodes be returned?
...
# Waiting for operators.utility.assemble_scalars_to_vectors_fc
# displacement_X_Y = static_simulation.displacement(
#     components=["X", "Y"], nodes=[1, 2, 3], load_steps=[1], sub_steps=[1]
# )
...
# Extract elemental X stresses for elements 1, 2 and 3 at set 1
stress_X = static_simulation.elemental_stress(
    components=["X"],
    elements=static_simulation.mesh._meshed_region.elements.scoping.ids.tolist(),
    set_ids=[1],
)
print(stress_X._fc)

# Extract elemental XY stresses for elements 1, 2 and 3 at set 1
stress_XY = static_simulation.elemental_stress(
    components=["XY"], elements=[141, 441, 741], set_ids=[1]
)
print(stress_XY._fc)
